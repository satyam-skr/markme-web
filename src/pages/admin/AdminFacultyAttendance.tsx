...
import { adminAttendanceApi, courseApi, facultyApi } from "@/lib/api";
...
export default function AdminFacultyAttendance() {
  ...
  const [onlyMarkedByFaculty, setOnlyMarkedByFaculty] = useState(false);
  ...
  const fetchAttendanceData = async () => {
    ...
    try {
      const data = await adminAttendanceApi.getFacultyAttendance({
  ...
  const fetchCourseAndFacultyInfo = async () => {
    if (!facultyId || !courseId) return;
    ...
    try {
      ...
      // Fetch faculty info
      const facultyData = await facultyApi.getFaculty(facultyId);
      setFaculty(facultyData.data.faculty);
    } catch (err) {
      console.error("Failed to fetch course or faculty info:", err);
    }
  };

  useEffect(() => {
    if (facultyId && courseId) {
      fetchCourseAndFacultyInfo();
      fetchAttendanceData();
    }
  }, [facultyId, courseId]);
  ...
  const attendanceMatrix = useMemo(() => {
      ...
      dateData.students.forEach(studentAttendance => {
        ...
        if (matrix[studentId]) {
          matrix[studentId][date] = {
            ...
            faculty: studentAttendance.faculty || {} as Faculty,
    ...
    // Apply "marked by faculty" filter if needed
    if (onlyMarkedByFaculty && facultyId) {
      Object.keys(matrix).forEach(studentId => {
        Object.keys(matrix[parseInt(studentId)]).forEach(date => {
          const attendance = matrix[parseInt(studentId)][date];
          if (attendance && attendance.faculty && attendance.faculty.id !== parseInt(facultyId)) {
            matrix[parseInt(studentId)][date] = null;
          }
        });
      });
    }
    ...
    return matrix;
  }, [attendanceData, filteredStudents, sortedDates, onlyMarkedByFaculty, facultyId]);
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `attendance_${course?.courseName?.replace(/\s+/g, '_') || 'course'}_${faculty?.firstName}_${faculty?.lastName}_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            {faculty && (
              <p className="text-sm text-muted-foreground">
                Faculty: {faculty.firstName} {faculty.lastName} ({faculty.department})
              </p>
            )}
          </div>
        </div>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4 items-end">
            ...
            <div className="flex items-center space-x-2">
              <Switch
                id="marked-by-faculty"
                checked={onlyMarkedByFaculty}
                onCheckedChange={setOnlyMarkedByFaculty}
              />
              <Label htmlFor="marked-by-faculty" className="text-sm">
                Only marked by this faculty
              </Label>
            </div>
          </div>
        </CardContent>
